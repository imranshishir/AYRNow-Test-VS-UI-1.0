...
**Notes:** Dev profile runs on port **8081**. Schema is owned by Flyway (no Hibernate create-drop in dev). For iOS Simulator, if `127.0.0.1` doesn’t reach the host, set API Base URL in the app (Settings, debug) to `http://<your-mac-lan-ip>:8081` (e.g. `ipconfig getifaddr en0`).
...
curl -H "Authorization: Bearer $TOKEN" http://localhost:8081/v1/me
```
...
curl -H "Authorization: Bearer $TOKEN" \
  "http://localhost:8081/v1/rent-board?propertyId=33333333-3333-3333-3333-333333333333"
```
...
# List tickets
curl -H "Authorization: Bearer $TOKEN" \
  "http://localhost:8081/v1/tickets?propertyId=33333333-3333-3333-3333-333333333333"
...
curl -X POST -H "Authorization: Bearer $TOKEN" \
  ...
  http://localhost:8081/v1/tickets
```

### Notifications

```bash
curl -H "Authorization: Bearer $TOKEN" http://localhost:8081/v1/notifications
```

### Payments (Stripe TEST or stub)

```bash
# Create payment intent (amount in dollars; backend converts to cents)
curl -X POST http://127.0.0.1:8081/v1/payments/intent \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"unitId":"44444444-4444-4444-4444-444444444444","amount":1500}'
```

When `STRIPE_SECRET_KEY` is not set, this endpoint uses a local stub and still writes `payments` and `ledger_entries`.  
When `STRIPE_SECRET_KEY` is set to a Stripe **test** key (`sk_test_...`), the backend creates a real Stripe PaymentIntent and returns its `clientSecret`.
